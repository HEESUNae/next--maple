...
import getUpdateDetail from '@/action/update/update-detail-action';

interface UpdateDetailProps {
  params: Promise<{ id: string }>;
}

export default async function UpdateDetail({ params }: UpdateDetailProps) {
  const { id } = await params;
  const { data, error } = await getUpdateDetail(id);
